    ...
    */
    ...
    if (init(gWindow, gRenderer, SCREEN_WIDTH, SCREEN_HEIGHT) >= 0)
    {
        ...
        while (!quit)
        {
            ...
            while (SDL_PollEvent(&e) != 0)
            {
                ...
                else if (e.type == SDL_MOUSEMOTION)
                {
                    ...
                    if (e.button.button == SDL_BUTTON_LEFT) {
                        goal_state << e.button.x - (SCREEN_WIDTH / 2), (-1)* (e.button.y - (SCREEN_HEIGHT / 2)), 0, 0, 0, 0;
                        quadrotor.SetGoal(goal_state);
                    }
                }
                    ...
                    if (e.key.keysym.sym == SDLK_p) {
                        ...
                        mp::xlim({ -SCREEN_WIDTH / 2, SCREEN_WIDTH / 2 });
                        mp::ylim({ -SCREEN_HEIGHT / 2, SCREEN_HEIGHT / 2 });
                        mp::title("trajektoria y(x)");
                        mp::subplot(2, 1, 1);
                        mp::plot(time, theta_history);
                        mp::title("theta(t)");
                        mp::show();
                        x_history.clear();
                        y_history.clear();
            ...
            if (i % 100 == 0 && state != goal_state) {
                x_history.push_back(state[0]);
                y_history.push_back(state[1]);
                time.push_back(theta_history.size() * dt);
